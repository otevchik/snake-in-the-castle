...
function renderPerksSelection() {
  ...
  if (!playerData || !playerData.inventory || playerData.inventory.length === 0) {
    container.innerHTML = '<p style="text-align: center; color: #a0a0c0; padding: 30px; grid-column: 1/-1;">No power-ups available. Buy some from the Shop!</p>';
    return;
  }
...
async function consumeSelectedPerks() {
  for (const perkId of selectedPerks) {
    await SupabaseSpace.consumeItem(TelegramApp.getUserId(), perkId);
  }
  playerData = await SupabaseSpace.getPlayer(TelegramApp.getUserId());
}
...
function maskPlayerName(name, isMe) {
  if (isMe) return name;
  if (!name) return 'P***t';
  const str = String(name);
  if (str.length <= 4) return str.charAt(0) + '**' + str.charAt(str.length - 1);
...
async function renderLeaderboard() {
  ...
  const myId = playerData?.telegram_id;
  
  container.innerHTML = '';
  
  if (!data || !data.players || data.players.length === 0) {
    container.innerHTML = '<p style="text-align: center; color: #a0a0c0; padding: 30px;">No pilots yet. Be the first!</p>';
    return;
  }
  
  data.players.forEach((player, index) => {
    const globalRank = (currentLeaderboardPage - 1) * 10 + index + 1;
    const isMe = myId && String(myId) === String(player.telegramId);
    ...
    const displayName = maskPlayerName(player.displayName, isMe);
    
    const item = document.createElement('div');
    item.className = `leaderboard-item ${rankClass} ${isMe ? 'current-player' : ''}`;
    item.innerHTML = `
      <span class="rank">${globalRank}</span>
      <span class="rank-icon">${rankIcon}</span>
      <span class="player">${displayName}${isMe ? ' 👈' : ''}</span>
...
async function renderShop() {
  ...
  caseSection.innerHTML = `
    <div class="case-item">
      ...
      <button id="openCaseBtn" class="open-case-btn" ${!canAfford ? 'disabled' : ''}>
        ${canAfford ? '🎲 Open Crate' : 'Not enough coins'}
      </button>
    </div>
  `;
  ...
  container.querySelectorAll('.buy-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      TelegramApp.hapticImpact('medium');
      const result = await SupabaseSpace.buyItem(TelegramApp.getUserId(), btn.dataset.item, parseInt(btn.dataset.price));
      if (result.success) {
...
async function openMysteryCrate() {
  const result = await SupabaseSpace.openCrate(TelegramApp.getUserId(), MYSTERY_CRATE.price, ALL_SHIPS);
...
async function equipShip(shipItem) {
  const result = await SupabaseSpace.equipShip(TelegramApp.getUserId(), shipItem.id);
  if (result.success) {
    playerData = result.player;
...
async function startNewGame() {
  try {
    currentGameSession = await SupabaseSpace.startGameSession(TelegramApp.getUserId());
  } catch (e) {
...
function draw() {
  ...
  asteroids.forEach(a => {
    ...
    ctx.fill();
    ctx.beginPath();
    ctx.arc(a.width/5, a.height/6, a.width/8, 0, Math.PI * 2);
    ctx.fill();
...
async function initApp() {
  ...
  try {
    ...
    if (!userId) {
      document.querySelector('.loading-content h2').textContent = 'Error: No user';
      return;
    }
    
    playerData = await SupabaseSpace.getPlayer(userId);
    
    if (!playerData) {
      playerData = await SupabaseSpace.createPlayer(userId, TelegramApp.user);
    }
    ...
    document.querySelector('.loading-content h2').textContent = 'Error loading';
  }
}
